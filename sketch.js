...
function showArrowsss(arrow, index) {
  arrow.display();
  if(arrow.body.position.x <= 50 || arrow.body.position.y >= height - 50 || arrow.body.position.x >= width - 300){
    Matter.World.remove(world,arrow.body);
...
for(var i = 0; i < playerArrows.length;i++){
  ...
  if(baseCollision.collided || archerCollision.collided || computerCollision.collided){
    console.log("PLayer are Collided");
...
function handleComputerArrowCollision() {
  ...
  for(var i = 0; i < computerArrows.length;i++){
    ...
    if(base1Collision.collided || archer1Collision.collided || playerCollision.collided){
      console.log("Computer are Collided");
